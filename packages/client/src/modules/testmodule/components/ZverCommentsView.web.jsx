...
import ZverCommentsForm from './ZverCommentsForm';

class ZverCommentsView extends React.PureComponent {
  static propTypes = {
  ...
  onSubmit = () => values => {
    const { comment, zverId, addComment, editComment, onCommentSelect } = this.props;

    if (comment.id === null) {
      addComment(values.content, zverId);
    } else {
  ...
  render() {
    const { zverId, comments, comment, t } = this.props;
    const columns = [
      {
    ...
    return (
      <div>
        ...
        <ZverCommentsForm zverId={zverId} onSubmit={this.onSubmit()} initialValues={comment} comment={comment} />
        <h1 />
...
export default translate('zver')(ZverCommentsView);
